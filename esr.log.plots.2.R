...
#test.vec = vector()
#table(dl$chr) is a vector of markers in each chr that will be used to make a vector of chromosome starts crudely 
#for (i in (1:length(table(dl$chr)))) {
#  test.vec[i] = sum(table(dl$chr)[1:i])
#}

abline(h=4, col="red")
abline(a=NULL,b=NULL,h=NULL,v=cumsum(rle(dl$chr)$lengths),lty=2, col="gray")

legend("topright",                           
  c("t0 RP", "t0 iESR", "t0 RiBi", "t30 RP", "t30 iESR", "t30 RiBi"),
  lty = 1,
  col = c("light green", "dark green", "light blue", "orange", "red", "yellow"), cex = .1)

axis(1, at = cumsum(rle(dl$chr)$lengths), labels = c(rle(dl$chr)$values[2:length(rle(dl$chr)$values)],"end"), las=2)
